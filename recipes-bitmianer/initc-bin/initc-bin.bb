...
do_install() {
		...
		if [ x"chanel3" == x"${chanel}" ]; then
			install -m 0644 ${S}/A8_FPGA_4000.rbf ${D}${bindir}/spitop_noncerev.rbf #fpga bin file
			echo "3c" >> ~/Desktop/a.log
		else
			echo "9c" >> ~/Desktop/a.log
            install -m 0644 ${S}/A8_FPGA_IIC.rbf ${D}${bindir}/spitop_noncerev.rbf #fpga bin file
        	fi
        	echo "S5+"		
